package dtx.example

/**
 * Represents a percentage value as a fraction between 0.0 and 1.0.
 * 
 * @property value The percentage value as a fraction between 0.0 and 1.0.
 */
@JvmInline
public value class Percent(public val value: Double) {

    /**
     * Creates a percentage from an integer value between 0 and 100.
     * 
     * @param intValue An integer between 0 and 100 representing a percentage.
     * @throws IllegalArgumentException if the value is not between 0 and 100.
     */
    public constructor(intValue: Int): this(intValue.toDouble() / 100.0) {
        require(intValue in 0..100) { "Value [$intValue] must be between 0 and 100" }
    }
}

public inline val Double.percent: Percent get() = Percent(this)
public inline val Int.percent: Percent get() = Percent(this)